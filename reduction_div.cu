...
// GPU kernel function with embedded reduction logic and debug prints
        ...
        if (t < stride) {
            switch (op) {
                ...
                case 'm':
                    ...
                    break;
                case 'M':
                    partialSum[t] = max(partialSum[t], partialSum[t + stride]);
                    break;
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < SIZE; i++) {
        data[i] = i;  // Set values from 1 to SIZE for easier debugging
        printf(" %d", data[i]);
    }
    printf("\n");
